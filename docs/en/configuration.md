...
## Core keys (new mode only)

### LLM

- `llm_candidates` (required)
  - ordered cross-provider fallback list
  - each item supports:
    - `enabled`
...
- `image_prompt_lite_content_chars` (first-paragraph truncation in `lite` mode)
...
- Uses OpenAI Images-compatible behavior (e.g. `/images/generations`).

### `dashscope_image`

- Uses DashScope official multimodal-generation API.
- `base_url` should be regional API root:
